...
(define* (nix-system->gnu-triplet-for-rust #:optional (system (%current-system)))
  (match system
  ...
  (package
    ...
    (source
     (origin
       ...
       (sha256
        (base32 checksum))))
    (native-inputs (cons* `("clang-source" ,(package-source clang-runtime-18))
                          `("gdb" ,gdb/pinned)
                          `("procps" ,procps)
                          (alist-replace "cargo-bootstrap"
                       ...
                       (lambda* (#:key inputs #:allow-other-keys)
                         (let ((gcc (assoc-ref inputs "gcc")))
                           ...
                           (setenv "CPLUS_INCLUDE_PATH"
                                   (string-join (cons (string-append (assoc-ref
                                                                      inputs
                                                                      "gcc")
...
")) #t))
                     (add-after 'configure 'fix-lld-compilation
                       (lambda* _
                         (invoke "cp" "-r"
                                 "src/llvm-project/libunwind/include/mach-o/"
                                 "src/llvm-project/lld/MachO/") #t))
                     (replace 'install
                       (lambda* (#:key outputs #:allow-other-keys)
                         (let* ((out (assoc-ref outputs "out"))
                                (sanitizers-dir (string-append (assoc-ref
                                                                outputs "out")
                                                 "/lib/rustlib/x86_64-unknown-linux-gnu/lib/"))
                                (from-dir
                                 "build/x86_64-unknown-linux-gnu/native/sanitizers/build/lib/linux/"))
                           (mkdir-p sanitizers-dir)
                           (copy-file (string-append from-dir
                                       "libclang_rt.asan-x86_64.a")
                                      (string-append sanitizers-dir
