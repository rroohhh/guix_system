  ...
  (package
    ...
    (source
     (origin
       (method url-fetch)
       (uri (string-append
             "https://dl.influxdata.com/influxdb/releases/influxdb2-" version
             "_linux_amd64.tar.gz"))
       (sha256
        (base32 "12322iaxfkcmbng20cxyq17q1ysg4v5p4dmcjcy4iys25sjwqb1a"))))
    (build-system copy-build-system)
    (arguments
     '(#:install-plan '(("usr/bin/influxd" "bin/influxd"))))
    (home-page "https://www.influxdata.com/")
    (synopsis "InfluxDB is a time series platform")
    (description "InfluxDB is a time series platform")
    (license #f)))
; MIT
  ...
  (package
    ...
    (source
     (origin
       (method url-fetch)
       (uri (string-append
             "https://dl.influxdata.com/influxdb/releases/influxdb2-client-"
             version "-linux-amd64.tar.gz"))
       (sha256
        (base32 "1rmzixan29f5g26snmiixc6cqar5gpilbgw6g2ybcqvlah2g6rm2"))))
    (build-system copy-build-system)
    (arguments
     '(#:install-plan '(("influx" "bin/influx"))))
    (home-page "https://www.influxdata.com/")
    (synopsis "InfluxDB is a time series platform - cli")
    (description "InfluxDB is a time series platform - cli")
    (license #f)))
; MIT
...
(define-public telegraf
  (package
    ...
    (source
     (origin
       ...
       (uri (string-append
             ...
             (string-replace-substring version "-" "~") "_linux_amd64.tar.gz"))
    ...
    (inputs `(("libc" ,glibc)
              ("patchelf" ,patchelf)))
    (arguments
     `(#:install-plan '(("usr/bin/telegraf" "bin/telegraf"))))
    (home-page "https://www.influxdata.com/")
    (synopsis
