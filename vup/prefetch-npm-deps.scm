...
(define nixpkgs
  (let ((version nixpkgs-version))
    (origin
      (method git-fetch)
      (uri (git-reference (url "https://github.com/nixos/nixpkgs")
                          (commit version)))
      (file-name (string-append "nixpkgs-" version "-checkout"))
      (sha256 (base32 "0b96wqvk3hs98dhfrmdhqmx9ibac4kjpanpd1pig19jaglanqnxr")))))
     ...
     (origin
       ...
       (uri (delay (with-imported-modules '((guix build utils))
                                          #~(begin
                                              ...
                                              (mkdir #$output)
                                              (copy-recursively (string-append #$nixpkgs
                                                                 "/pkgs/build-support/node/fetch-npm-deps")
                                                                #$output)))))))
    (build-system cargo-build-system)
    (native-inputs (list pkg-config))
    (inputs (append (list curl nghttp2 openssl zlib)
                    (import-rust-lock source)))
    (home-page "FILLMEIN")
    (synopsis "FILLMEIN")
    (description (beautify-description "FILLMEIN"))
    (license #f)))

(define-public prefetch-npm-deps-bin
  (file-append prefetch-npm-deps "/bin/prefetch-npm-deps"))
